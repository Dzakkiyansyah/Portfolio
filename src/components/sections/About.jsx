...
const highlights = [
  { icon: <FiBriefcase className="w-8 h-8" />, title: "1+ Years", subtitle: "Experience" },
  { icon: <FiZap className="w-8 h-8" />, title: "10+ Projects", subtitle: "Completed" },
  { icon: <FiCode className="w-8 h-8" />, title: "Frontend", subtitle: "Focus" }
];
  ...
  return (
    <section id="about" className="relative py-24 bg-transparent">
      <div className="container grid items-center gap-10 px-6 mx-auto md:grid-cols-2 md:gap-20">
        ...
        >
          <p className="mb-4 font-semibold text-primary drop-shadow-sm">A Glimpse About Me</p>
          ...
          <p className="mt-6 text-base md:text-lg text-subtle dark:text-subtle-dark drop-shadow-sm">
            Fokus saya adalah menciptakan aplikasi web yang cepat, responsif, dan mudah digunakan, mengubah ide menjadi kenyataan digital yang fungsional.
          </p>
          <div className="mt-8">
            <Link to="/about" className="inline-flex items-center text-lg font-bold transition-colors text-primary group drop-shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-1">
          {highlights.map((item, index) => (
            ...
            >
              <div className="p-4 text-white rounded-lg bg-primary">{item.icon}</div>
              <div>
